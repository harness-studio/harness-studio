...
- **Spec-driven design (spec before code).** No code is touched until the spec is **locked**: the problem, the acceptance criteria, and the ADR (decisions) are settled and human-approved. This is stronger than TDD — TDD says *test before code*; spec-driven says *spec/ADR before test or code*. There are **two human gates**: the project-level **architecture lock** (`hssd architecture approve`) — the system proposes the ADR (`hssd overview architect`), an adversary advises, the engineer iterates and locks it as an immutable, versioned decision (`docs/adr/ADR-vN.md`); decomposition (`overview split`) and `engage` are gated behind it — and the per-story **Spec Lock** (end of Phase 2), which Phase 3 cannot start before. Exception: notebooks (exploratory).
...
- **TDD for applications — enforced, not just stated.** Test-first: a failing test, then the code. **Only exception: notebooks** (exploratory/research — not test-first). This is a *hard gate* in `hssd engage`, not a hope: P3 splits into **P3a Red** — a dedicated `test-author` role writes the tests from the *locked* acceptance criteria and the CLI **runs them itself; they must fail** (tests that pass with no implementation are vacuous → BLOCK) — and **P3b Green** — the builder implements until the CLI sees them pass. Both the red and green test outputs are captured (`​.harness/engagements/<id>/tests-red.log` / `tests-green.log`) as auditable evidence. Maker ≠ checker holds: `test-author` writes the tests, `backend-dev`/`frontend-dev` write the code, the P4 adversaries try to break both. The test command defaults to `uv run pytest` (override with `--test-cmd`).
...
- **A blocker proposes; it never just stops (loop-forward).** Any adversary or gate that BLOCKs MUST return, with each finding, **1-3 candidate resolutions and one `recommended`** — the smallest defensible way forward. The process never dead-ends on "this is ambiguous": there is always at least one hypothesis to proceed with. The human picks/edits (recorded as ADR assumptions via `hssd engage --answers`), or — under earned autonomy — the system takes the recommended one and retries (`hssd engage --accept-recommended`). This is loop engineering: hypotheses + adversarial evaluation keep the loop moving, always with an answer (or several). **Bounded loop-forward (two ceilings + a converge-by-exhaustion exit).** "Never stops" must not mean "never ends": each gate retries at most a fixed number of rounds (`max_retry`, default 2 → 3 attempts) and every engagement carries a per-run AI-call/budget ceiling (`--max-calls` default 40, `--budget` USD). When a gate's retries are *exhausted* under `--accept-recommended`, it does **not** dead-end — it accepts the final round's recommendations, marks them **convergence by exhaustion**, and carries the accumulated assumptions forward to the **Spec Lock human gate**, the natural arbiter. Field-learned (LOC-7, fleet telemetry): a single story pulled from a multi-story system may never fully converge in isolation — the skeptic keeps surfacing *real* cross-story dependencies (shared table ownership, who writes which column). That is signal for the human reviewing the locked spec, not a reason to stall the loop or burn tokens forever. The human (or `--auto`, testing only) still signs off at Spec Lock; autonomy resolves *findings*, it never bypasses the *gate*.
...
### Cross-cutting engineering skills (find-and-propose guards)

Five blessed skills encode recurring correctness lessons and are loaded as **find-and-propose checklists** — the architect/builder load them to be correct by construction; the adversaries load them to catch violations and propose the fix (this is "fix the harness, not the code" — each was a defect caught by hand once, now a standing guard). They are: **`sqlite-concurrency`** (WAL + `busy_timeout`, atomic counters, `BEGIN IMMEDIATE` for check-then-act run in a single executor slice, `ON CONFLICT DO UPDATE` never `INSERT OR REPLACE`, single-writer ownership per column), **`sql-indexing`** (index every FK + every filter/range path; verify with `EXPLAIN QUERY PLAN`), **`datetime-utc`** (everything UTC — backend/DB/data lake; only the frontend localizes; tz-aware ISO 8601; reject naive), **`api-conventions`** (response shapes, HTTP status codes, closed `[since, until]` intervals, sort+tiebreak, pagination-by-tier, structured `422`), **`resilience`** (bounded timeouts, bounded retries with backoff, idempotent retried writes, fail-fast — the harness instinct at runtime), and **`push-over-pull`** (event-driven push — WebSockets/SSE/pub-sub/Kafka/webhooks — is the default; polling is a justified, ADR-recorded exception). Wired via `ROLE_SKILLS` to architect, architecture-adversary, backend-dev, test-adversary (and the datetime/API/push ones to frontend-dev).
